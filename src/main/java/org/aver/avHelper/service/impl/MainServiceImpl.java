...
//			file.mkdir();
    ...
    public Movie findMovie(String fanhao) {
        ...
        generateNfoInfo(movie, "D:\\data");
        downloadImg(movie, "D:\\data");
     ...
     */
    @Override
    public void generateNfoInfo(Movie movie, String path) {
        //创建info文件
        org.dom4j.Element tempEle = null;
        org.dom4j.Document document = DocumentHelper.createDocument();
        org.dom4j.Element movieEle = document.addElement("movie");
        ...
        if (movie.getActorName() != null && movie.getActorName().size() > 0) {
            for (String name : movie.getActorName()) {
                ...
                org.dom4j.Element nameEle = tempEle.addElement("name");
                nameEle.setText(name);
                org.dom4j.Element typeEle = tempEle.addElement("type");
...
//            String infoName = movie.getNewName().substring(0, movie.getNewName().lastIndexOf("."));
            String infoName = movie.getSorttitle();
            String infoPath = path + "/" + infoName + ".nfo";
...
//			writer.setEscapeText(false);
     ...
     *
     * @param movie 电影信息
     * @param movieDirPath  生成的目标目录
     */
    @Override
    public void downloadImg(Movie movie, String movieDirPath) {
        //下载图片到对应的文件夹
        // 图片扩展名
        ...
        if (movie.getFanartsPicSite() != null && movie.getFanartsPicSite().size() > 0) {
            // 多线程代码
            ThreadPoolExecutor executor = new ThreadPoolExecutor(30, 30, 60000,
                    TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>());
            new File(movieDirPath + "/"+ movie.getSorttitle()+"-extrafanart").mkdir();
            String tempPicName;
            String[] array;
            for (String site : movie.getFanartsPicSite()) {
                ...
                if (array.length == 2 && array[1].length() == 1) {
                    tempLocalPath = movieDirPath + "/"+ movie.getSorttitle()+"-extrafanart/" + array[0] + "-0" + array[1] + tempExtName;
                } else {
                    tempLocalPath = movieDirPath + "/"+ movie.getSorttitle()+"-extrafanart/" + tempPicName;
                }

                if (neededToDownload(new File(tempLocalPath))) {
                    final String finalSite = site;
                    final String finalPath = tempLocalPath;
                    executor.execute(() -> ImageDownload.downloadByProxy(finalSite, finalPath));
                }
